    ...
    module PasswordArchivable
      ...
      def password_archive_included?
        ...
        if self.class.deny_old_passwords > 0 and not self.password.nil?
          old_passwords_including_cur_change = self.old_passwords.desc(:id).limit(self.class.deny_old_passwords).to_a
      ...
      def archive_password
        if encrypted_password_changed? && (encrypted_password_change && !encrypted_password_change.first.blank?)
